...
共同編集データはSupabase PostgreSQLを正本とし、ブラウザから`@supabase/supabase-js`で読み書きする。`shared_pages`は固定作戦ページ、`shared_memos`は時系列フィードを担当する。両テーブルのPostgres Changesを購読し、通知時に再取得する。

認証は導入せず、RLSで`anon`ロールへ必要な操作を許可する。Supabase未設定時は静的な初期値を表示するが、書き込み操作は無効化し、ローカル保存を共有済みデータとして扱わない。
